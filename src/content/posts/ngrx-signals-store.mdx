...
@Component({
  selector: 'app-hello',
  standalone: true,
  template: ` <h1>Hello {{ helloStore.firstName() }}!</h1> `,
...
## How the NgRx Signal Store works
...
@Component({
  selector: 'app-user',
  standalone: true,
  template: ` <h1>Country: {{ userStore.user.address.country() }}!</h1> `,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
...
Because unintentional overriding might lead to issues that are difficult to detect, the NgRx team has already introduced a warning which is displayed in development mode if a user tries to override SignalStore members.
...
**_You can find the full implementation of the above store [here](https://github.com/stefanoslig/angular-ngrx-nx-realworld-example-app/blob/main/libs/articles/data-access/src/lib/article.store.ts)_**

### withProps

In the code snippet above you can see how we pass the dependencies we want to use in the `withMethods` as parameters in the method. What if we wanted to use the same dependencies later on another method (e.g withHooks)? Then we would have to duplicate the code to include these dependencies. We can avoid this issue by using the new core method `withProps` which was introduced in version 19 of the NgRx Store. The main responsibility of this new method is to share properties that are not computed state, methods or state (we cannnot add them in any of the other core methods) to the rest of the store. The example above could be rewritten like this:
...
export const ArticleStore = signalStore(
  ...
  withMethods((store) => ({
    getArticle: rxMethod<string>(
      pipe(
        switchMap((slug) =>
          store._articlesService.getArticle(slug).pipe( // <== the ArticlesService is shared here and in withHooks
          ...
          ),
        ),
        ...
  withHooks((store) => {
    return {
      onInit() {
        store._articlesService.getArticle(...) // <== ArticlesService can be used here as well
      },
    };
  }),
```

Apart from dependencies in the withProps, we can expose signals as Observables so we don't have to transform them every time to Observables in the consumers of the store.
...
<Aside type="caution">
  I would strongly advice on avoid using the resource or rxResource APIs right
  now in your NgRx Stores. Apart from the obvious fact that it is an
  experimental feature, it is a half told story for the moment. You can use the
  resource to fetch data but only when the full story is told regarding this
  feature you will be able to use it for PUT, POST, DELETE requests. Because of
  this limitation, you would have to mix resources with rxMethod for example
  wich would make your store more complex.
</Aside>
...
- [Example app using NgRx Signal Store](https://github.com/stefanoslig/angular-ngrx-nx-realworld-example-app)
...
>
> \[2\]:Signal Store docs [https://ngrx.io/guide/signals/signal-store](https://ngrx.io/guide/signals/signal-store)
>
